...
def run_forward_distance_experiment(
    ...
    scenario_name = getattr(args, "scenario_name", "Step01_Forward_Distance")
    scenario = (
        ScenarioConfig(scenario_name)
    ...
    run_config = {
        ...
        "current_policy": (
            "Apply the current before every simulation step; the local HoloOcean "
            "2.2.2 API documents current as an enqueued command and does not state "
            "that one call persists."
        ),
        "dt_policy": (
        ...
        "sensor_policy": {
            ...
            "IMU": "available for future estimation, not used in this experiment",
            "Depth": "available for future depth control, not used in this experiment",
    ...
    with holoocean.make(
        ...
        samples.append(
            build_sample(
                ...
                stop_command,
                dvl_sample,
                current_vector if current_enabled else None,
            )
        )
    ...
    summary.update(
        {
            ...
            "current_warning": current_config.warning(),
        }
    )
...
def build_distance_validation(
    ...
    if pose_euclidean > abs(pose_forward) * (1.0 + DISTANCE_WARNING_RATIO):
        warnings.append(
            "Pose Euclidean displacement is much larger than Pose forward "
            "displacement, indicating significant drift."
        )
...
def add_summary_aliases(summary: dict) -> None:
    """Add stable field aliases used by Step 2 without removing Step 1 keys."""

    summary.update(
        {
            "target_distance": summary["target_distance_m"],
            "dvl_estimated_distance": summary["dvl_estimated_distance_m"],
            "pose_forward_displacement": summary["pose_forward_displacement_m"],
            "pose_euclidean_displacement": summary["pose_ground_truth_displacement_m"],
            "longitudinal_error": summary["longitudinal_error_m"],
            "absolute_longitudinal_error": summary["absolute_longitudinal_error_m"],
            "absolute_error": summary["absolute_distance_error_m"],
            "lateral_drift": summary["lateral_drift_m"],
            "final_position_error": summary["final_position_error_m"],
            "duration": summary["duration_s"],
            "sample_count": summary["num_samples"],
            "average_pose_speed": summary["average_pose_speed_mps"],
            "max_dvl_forward_velocity": summary["max_dvl_forward_velocity_mps"],
            "mean_dvl_forward_velocity": summary["mean_dvl_forward_velocity_mps"],
        }
    )
...
def print_warnings(summary: dict) -> None:
    current_warning = summary.get("current_warning", "")
    if current_warning:
